...
- `gpui-node-graph`: one generic `NodeGraph<T, N, P, C>` retained-mode view and input adapter. The adapter and core share one `GraphEvent` vocabulary (`EditorEvent` is a compatibility alias). Consumers may reconcile domain data, supply arbitrary `NodeBodyRenderer` GPUI element trees, and handle connection/create/history hooks while the view owns transient gestures. The fixed shell passes viewport zoom to rich body renderers and preserves typed port overlays.
